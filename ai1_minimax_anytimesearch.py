# -*- coding: utf-8 -*-
"""ai1_minimax_anytimesearch.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1nOEo4PFzmnbajM8nMX11wi3Q4gva9Vw5
"""
    ...
    for (dr, dc) in DIRECTIONS:
        for r in range(board.size):
            for c in range(board.size):
                # Check that the last cell of the window is still on the board
                er, ec = r + 4 * dr, c + 4 * dc
                if not (0 <= er < board.size and 0 <= ec < board.size):
                    continue

                # Count p_cnt and o_cnt for this window, then update score.
                p_cnt = 0
                o_cnt = 0
...
def _count_dir(board: Board, r: int, c: int,
    ...
    for sign in (1, -1):
        ...
        # Walk in direction sign * (dr,dc) while in bounds and cell == player.
...
def quick_score(board: Board, r: int, c: int, player: int) -> int:
    ...
    Defense weights (opponent consecutive count -> weight):
        ...
        4 → 0.95 (opponent 1-away → block unless YOU are also 1-away)
        3 → 0.90
        2 → 0.70
        1 → 0.50
...
def get_best_move(board: Board, player: int,
    ...
    Algorithm:
        For depth = 1, 2, 3, … up to MAX_DEPTH:
            Run a full depth-limited search with _minimax.
            If the search COMPLETES within time_limit:
                -> update best_move and best_depth
            Else (search returned None or time ran out):
                -> stop and return best_move from last completed depth.

    This guarantees we always have an answer (from depth d-1) even if
    the depth-d search is cut short (the "anytime" property).

    INPUT:
        board       : Board
        player      : 1 or 2
        time_limit  : wall-clock budget in seconds  (default 15 s)
    OUTPUT:

    CONSTANTS you may use:
        MAX_DEPTH = 20  (iterative deepening stops here even with time left)
    """
    MAX_DEPTH  = 20
    start      = time.time()
    opponent   = 3 - player
    ...
    # ------------------------------------------------------------------
    # PRE-CHECK 3 — Own open-4: one move from winning
    # ------------------------------------------------------------------
    ...
    for depth in range(1, MAX_DEPTH + 1):
        ...
        # Run one full pass at this depth from the root
        candidate_move = None
        ...
        for (r, c) in sorted_moves:
            ...
            # Call _minimax for the remaining depth-1.
            # If result is None → timed_out = True; undo and break.
            # Otherwise update candidate_move / candidate_score.
            # -----------------------------------------------------------------
            score = _minimax(board, depth - 1, -math.inf, math.inf, False, player, (r, c), start, time_limit)
            board.undo_move(r, c)
            ...
            if candidate_score >= WIN_SCORE:
                ...
                best_depth = depth
                timed_out = False  # treat as completed
                break
...
if __name__ == "__main__":
    ...
    # 1. score_window
    # ------------------------------------------------------------------
    assert score_window(3, 0) == SCORES[3], "open-3 should score SCORES[3]"
    assert score_window(3, 1) == 0,         "blocked window should score 0"
    assert score_window(0, 0) == 0,         "empty window should score 0"
    assert score_window(5, 0) == SCORES[5], "5-in-row should score WIN"
    ...
    # ------------------------------------------------------------------
    # 3. evaluate — gap pattern  X X _ X X  should score as 4, not 2+2
    # ------------------------------------------------------------------
    board.make_move(0, 0, 1)
    board.make_move(0, 1, 1)
    board.make_move(0, 3, 1)
    board.make_move(0, 4, 1)

    score_gap = evaluate(board, 1)
    # The window cols 0-4 has p_cnt=4 → SCORES[4]; must be at least that
    assert score_gap >= SCORES[4], "gap pattern X X _ X X should score >= SCORES[4]"
    board.grid[:] = 0
    ...
    assert evaluate(board, 1) == -evaluate(board, 2), "evaluate must be symmetric"
    board.grid[:] = 0
    print("PASS: evaluate symmetry")

    # ------------------------------------------------------------------
    # 5. get_best_move — block opponent's open 4
    # ------------------------------------------------------------------
    for col in range(4):
        board.make_move(0, col, 2)
    move = get_best_move(board, 1, time_limit=15.0)
    assert move in ((0, 4), (0, -1)) or move[0] == 0, \
        f"AI should block row 0, got {move}"
    board.grid[:] = 0
    print("PASS: get_best_move blocks opponent 4")

    # ------------------------------------------------------------------
    # 6. get_best_move — take winning move immediately
    # ------------------------------------------------------------------
    for col in range(4):
    ...
    # ------------------------------------------------------------------
    # 7. get_best_move — block opponent open-3
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # 8. get_best_move — anytime: must return within time limit
    # ------------------------------------------------------------------
    import random
    random.seed(42)
    for _ in range(10):
